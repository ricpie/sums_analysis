...
```{r load_data, warning=FALSE, echo=FALSE}
  ...
  if (reimport_data == TRUE) {
    #Rprof("rprofout.out") #Analyze code, it is quite slow due to gsubfn checking for naming errors... but only need to do it occasionally.
...
sumsarized_filtered <- filter_sumsarized(sumsarized,metadata,bad_files,HHID_remove) #HHID is grabbed from the filename.
```
 ...
 for (i in unique(sumsarized_filtered$file_indices)) {
   ...
   if (dim(temp)[1]>1) {
     ...
     time_difference <- time_difference[!is.na(time_difference)] #
      ...
      if (is.na(as.POSIXct(temp$datetime_removal[1])) | is.na(as.POSIXct(temp$datetime_placed[1]))){  #If date is NA, use the file's dates.  They have already been midnight-trimmed in the load_data.r function.
             ...
             datetime_placed_temp = min_time_file
             datetime_removal_temp = max_time_file  
       } else if (as.POSIXct(temp$datetime_removal[1])>end_date_range | as.POSIXct(temp$datetime_placed[1])<start_date_range) {   #datetime_placed is changed to the local file value if it is below the start_date_range, and datetime_placed is too, according to the end_date_range
             datetime_placed_temp = min_time_file
             datetime_removal_temp = max_time_file  
      ...
      cooking_events <- rbind(cooking_events,
       ...
       cooking_events <- rbind(cooking_events,
                  ...
                  end_time=as.POSIXct(temp$datetime[1]), group=as.factor(temp$group[1]),
...
#Clean up cooking events. Add better variable names for stoves, and units.
 ...
 # grouped_summary <- cooking_events %>% dplyr::group_by(group,stove_descriptions) %>%
...
```{r create_flags}
  ...
  ok_cooking_events <-  dplyr::filter(cooking_events,!is.na(stove_descriptions)) %>%
                          ...
                          dplyr::select(filename,fullsumsarizer_filename,start_time,end_time,duration_minutes,HHID,stove,logger_id,group,
...
for (i in 1:length(unique(ok_cooking_events$filename))) {
    ...
    if (dim(temp)[1]>0) {    
        ...
        merged.data <- merge(all.dates.frame, temp, all=T) %>%
            tidyr::fill(filename,fullsumsarizer_filename,HHID,stove,logger_id,group,stove_use_category,
...
events_summary <- dplyr::filter(ok_cooking_events,!duplicated(fullsumsarizer_filename)) %>% #Remove duplicate files because we only one to get the logging duration from a single deployment
        ...
        dplyr::distinct(fullsumsarizer_filename, .keep_all = TRUE) %>%
...
#Summary of all cooking events.  Group by group and stove type to get the stats on durations of all the cooking events.
 grouped_summary_a <- ok_cooking_events %>% dplyr::group_by(group,stove_descriptions) %>%
...
grouped_summary_b <-  dplyr::filter(ok_cooking_events,!duplicated(filename)) %>% #Remove dup filenames to get these stats below without duplication.
      ...
      dplyr::summarize(days_logged_per_group_by_stove=sum(logging_duration_days, na.rm = TRUE))
                       #,nfiles=n())
        
#Get average of average daily uses by hh and stove
grouped_summary <- events_summary %>% dplyr::group_by(group,stove_descriptions) %>%
      ...
      dplyr::left_join(grouped_summary_a,by = c("group", "stove_descriptions")) %>%
      dplyr::left_join(grouped_summary_b,by = c("group", "stove_descriptions")) 
...
field_timeseries_plot(sumsarized_filtered, "stove_temp", "datetime", "HHID", "stove","qc") 
...
field_timeseries_plot(dplyr::filter(sumsarized_filtered,grepl("ok",qc)),"stove_temp", "datetime", "HHID", "stove","qc") 

```
# Plot data time series, removing flagged files and households and houses without metadata.
* Plot usage rates (uses/day) by stove type, and region
```{r plot_good_timeseries, fig.width=10, fig.height=50}
...
field_timeseries_plot(dplyr::filter(sumsarized_filtered,grepl("ok",qc)) %>% dplyr::filter(!is.na(stove_type)),
...
files_without_metadata <- unique(dplyr::filter(sumsarized_filtered,is.na(stove_type)) %>% dplyr::select(fullsumsarizer_filename))   #Note the name of files for which there is there is no matching filename in the tracking sheet, ergo no metadata from the tracking sheet. Not currently being filtered out, but this should be attended to by fixing any data files/tracking entries that appear in this variable.  Make sure that the actual file name matches what is listed in the tracking sheet.  Files names should also be unique.
files_without_metadata
```
...
```{r plot_stove_usage1, fig.width=6, fig.height=4}
...
# #Plot average uses per day with each stove for each household, by group.
...
# g1
# ggsave(filename="../figures/AverageUsesPerDay_byGroup.png", plot=g1,width = 10, height = 5)
...
g1
ggsave(filename="../figures/AverageUsesPerDay_byGrouppadded.png", plot=g1,width = 10, height = 5)


#Plot uses per day, by group, using the padded data set. (one data point per day-household-stove combo)
g1<- dplyr::group_by(ok_cooking_events_padded, stove_descriptions, day_month_year, HHID, group) %>%
  ...
  labs(y="Number of uses by day",x="") + 
  theme(legend.title = element_blank())
g1
ggsave(filename="../figures/UsesPerDayDistribution_byGrouppadded.png", plot=g1,width = 10, height = 5)

```

```{r plot_stove_usage2, fig.width=6, fig.height=4}
...
g4<-  dplyr::group_by(ok_cooking_events_padded,stove_descriptions,HHID,group) %>%
  ...
  geom_jitter(height = 0,width = 0.2,alpha = 0.25) +
  stat_summary(fun.data = give.n, geom = "text") + 
  labs(y="Average minutes used per event",x="") + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
g4
ggsave(filename="../figures/AverageTimeUsedPerEvent.png", plot=g4,width = 10, height = 5)
...
plotMonthlyFunc <- function(x,y, na.rm = TRUE, ...) {
    monthlyboxplots <- ggplot(dplyr::filter(x,grepl(y,group,ignore.case=TRUE)), aes(x=month_year, y = events_per_day_month,color = stove_descriptions)) + 
...
aaaa <- dplyr::group_by(ok_cooking_events,stove_descriptions,group,day_month_year) %>%
        ...
        dplyr::distinct(group,stove_descriptions,day_month_year, .keep_all = TRUE)  #Get only one representative row from each stove day.


plotStoveDaysFunc <- function(x, na.rm = TRUE, ...) {
  #Calculate percent stove-days.
  g7 <- ggplot(
      asdf<-dplyr::group_by(ok_cooking_events_padded,day_month_year,stove_descriptions) %>%
                dplyr::distinct(HHID,stove_descriptions,day_month_year, .keep_all = TRUE) %>% #No duplicate events from the same hh contributing to stove-days.
                 ...
                 dplyr::mutate(days_used_day_stove_grouped = 100*sum(use_flag, na.rm=TRUE)/sum(!use_flag, na.rm=TRUE)), 
...
plotStoveDaysFunc(events_summary_stovedays)
...
#Overall usage fraction by group.  
 g12<- dplyr::select(ok_cooking_events_padded,stove_descriptions,day_month_year,duration_minutes) %>%
...
#Usage time series by minutes used each day, by group and stove type  
 g12<- dplyr::select(ok_cooking_events_padded,stove_descriptions,day_month_year,duration_minutes,group) %>%
  group_by(stove_descriptions) %>%
  summarise(avg=mean(duration_minutes,na.rm=TRUE)) %>%
  dplyr::mutate(usefraction = avg/sum(avg)) %>%
  ggplot(aes(x=1, usefraction,label=sprintf("%0.2f", round(usefraction, digits = 2)))) +
