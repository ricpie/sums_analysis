...
# #Initialize the cooking_events frame.
 ...
 for (i in unique(sumsarized_filtered$file_indices)) {
   ...
   if ((max_time_file>end_date_range | min_time_file<start_date_range) | metadata_date_ignore == 1) {   #datetime_placed is changed to the local file value if it is below the start_date_range, and datetime_placed is too, according to the end_date_range
...
```{r plot_timeseries, fig.width=8, fig.height=4}
...
* Plot usage rates (uses/day) by stove type, and region
```{r plot_nonflagged_timeseries, fig.width=8, fig.height=4}
...
```{r boxplots_usage, fig.width=8, fig.height=5}
...
```
```{r boxplots_usage_region, fig.width=8, fig.height=7}
#Plot average minutes used per day for each stove-household combination
...
# Time trend plots
```{r plot_time_trends, fig.width=8, fig.height=6}
...
```{r plot_usage_fractions, fig.width=7, fig.height=4}
...
```{r plot_monitoring_diagnostics, fig.width=7, fig.height=4}
...
```{r plot_stacking, fig.width=7, fig.height=4}
...
```{r plot_simultaneous, fig.width=7, fig.height=4}
