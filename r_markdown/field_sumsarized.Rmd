...
```{r define_variables}
  ...
  parameter_filename <- "parameter_file_Nigeria_AfDB.R"  #Format example is :"parameter_file_HCCP_Ethiopia.R"
  
  source(paste0('../r_files/',parameter_filename)) #OG Codebase
  
  source('../r_files/parameter_file_ICDDRB.R') #OG Codebase
```
...
```{r load_data, warning=FALSE}
  ...
  if (exists("path_tracking_sheet_json") && !is.na(path_tracking_sheet_json)) {
      saveRDS(load_meta_json(path_tracking_sheet_json), file = "~/Dropbox/Kigeme SUMs Analysis/SUMS Analysis R Code/r_files/jsonmetadata.RDS")
  ...
  metadata <- filter_metadata_fun(metadata)
...
# gg <-field_timeseries_plot(dplyr::filter(sumsarized_filtered,grepl("ok",qc)),"stove_temp", "datetime", "HHID", "stove_descriptions","qc") 
# gg
```
...
# Time trend plots
```{r plot_time_trends, fig.width=7, fig.height=4}
