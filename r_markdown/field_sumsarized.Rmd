...
if (( reimport_data == TRUE) |  (!file.exists("../r_files/sumsarized_filtered.RDS"))) {
  sumsarized_filtered <- filter_sumsarized(sumsarized,metadata,bad_files,HHID_remove,
                                           project_name,stove_codes) #HHID is grabbed from the filename.
...
#Define grouping based on the stove_group_codes variable.  Didn't put it in filter function because this is faster.  Need to change grouping in the parameter file
sumsarized_filtered <-dplyr::left_join(sumsarized_filtered,stove_group_codes,by = "group") %>%
  dplyr::mutate(group = as.factor(stove_groups))
...
```{r list_orphaned_files, fig.width=10, fig.height=80,echo=TRUE}
...
ok_cooking_events_padded <- ldply(unique(ok_cooking_events_unfiltered$fullsumsarizer_filename),pad_fun,ok_cooking_events_unfiltered,.parallel = TRUE) 
...
#Filter the dataset.  Kept 'bad' and short events to get the full padded dataset.  Now can remove them
...
stats_grouped_by_hhid_stove_region <- dplyr::group_by(ok_cooking_events_padded,stove_descriptions,HHID,group,region,logger_id) %>%
  ...
  dplyr::summarise(events_per_day=sum(use_flag,na.rm=TRUE)/length(unique(day_month_year)),
                   ...
                   start_date=median(datetime_placed),
                   end_date=median(datetime_removal))
kable(stats_grouped_by_hhid_stove_region, digits=2)
...
stats_by_stove_group_region <- dplyr::group_by(stats_grouped_by_hhid_stove_region,stove_descriptions,group,region) %>%
  dplyr::summarise(mean_events_per_day=mean(events_per_day, na.rm = TRUE),
                   ...
                   house_stoves=n()) %>%
  dplyr::filter(!is.na(stove_descriptions)) 
kable(stats_by_stove_group_region, digits=2)
...
for (i in 1:length(uniqueHHIDs)){
  gg <- field_timeseries_plot(dplyr::filter(sumsarized_filtered,HHID == uniqueHHIDs[i]) %>% dplyr::filter(grepl("ok",qc)) %>%
                              ...
                              , "stove_temp", "datetime", "HHID", "stove_descriptions","state")
  print(gg)
}
```
...
give.mean_minutes <- function(x){   return(c(y =mean(x)+10, label = round(mean(x),digits=2)))}
...
g1 <- ggplot(grouped_data_means,aes(x=stove_descriptions, y = avg_events_per_day)) +
  ...
  facet_grid( ~ group,scales = "free", space = "free", labeller = label_wrap_gen(width = 12, multi_line = TRUE)) +
  labs(y="Eventos por dia en promedio",x="") + 
  ggtitle("Eventos por dia en promedio for each household-stove combination") + 
...
g111<- dplyr::group_by(ok_cooking_events_padded, stove_descriptions, day_month_year, HHID, group) %>%
  ...
  #facet_grid(stove_use_category~group,scales = "free", space = "free") + 
  #stat_summary(fun.data = give.n, geom = "text") + 
  facet_grid( group ~ stove_descriptions,scales = "free", space = "free", labeller = label_wrap_gen(width = 12, multi_line = TRUE)) +
  labs(y="Days",x="Uses per day") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + 
  theme(legend.title = element_blank())
g111
...
#                shape=18, size=4.5,alpha = 0.5)+
#   stat_summary(fun.data = give.mean_minutes, geom = "text",colour="blue") + 
#   facet_grid( group~region,scales = "free", space = "free", labeller = label_wrap_gen(width = 12, multi_line = TRUE)) +
...
g5 <- ggplot(ok_cooking_events, aes(x=day_of_week, y = duration_minutes)) + 
  ...
  facet_wrap( group ~ stove_descriptions,scales = "free_x", labeller = label_wrap_gen(width = 12, multi_line = TRUE)) +
...
monthlyboxplots <-    ggplot(stats_grouped_by_hhid_stove_month,aes(x=month_year, y = events_per_day,color = stove_descriptions)) +
  ...
  labs(y="Events/day",x="") + 
  facet_wrap( group ~ stove_descriptions,scales = "free_x") +
...
g12<- dplyr::group_by(ok_cooking_events_padded,stove_descriptions,day_month_year,group) %>%
  ...
  facet_grid( group~stove_descriptions ,scales = "free", space = "free", labeller = label_wrap_gen(width = 12, multi_line = TRUE)) +
  ggtitle("Average event duration") + 
  theme(legend.title = element_blank()) + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  labs(y="Minutes per event",x="") 
g12
...
g13<- dplyr::group_by(ok_cooking_events_padded,stove_descriptions,day_month_year,HHID) %>%
  ...
  geom_smooth()+
  geom_point(alpha = 0.25)+
  facet_grid( ~stove_descriptions ,scales = "free", space = "free", labeller = label_wrap_gen(width = 12, multi_line = TRUE)) +
  ggtitle("Daily average use by stove") + 
  theme(legend.title = element_blank()) + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
labs(y="Minutes per day",x="") 
g13
...
#Usage fraction time series by duration. Includes. all hh.  todo: We could filter out houses with fewer than x sums to reduce any potential bias from a certain sum type burning up or something.
#This plot looks at 
g10<- dplyr::select(ok_cooking_events_padded,stove_descriptions,start_time,duration_minutes) %>%
  ...
  ggtitle("Usage fraction by durations") + 
  labs(y="Usage fraction",x="")  +
  scale_fill_brewer(palette="Blues")+
  theme(legend.title = element_blank())
g10

#Usage fraction time series by daily cooking time.
gg10<- dplyr::select(ok_cooking_events_padded,stove_descriptions,start_time,duration_minutes,group,HHID,day_month_year) %>%
  ...
  geom_bar(aes(fill = stove_descriptions), 
           ...
           stat = 'identity', 
           position = "fill",size=0) +
  ggtitle("Usage fraction by daily cooking time") + 
  labs(y="Usage fraction",x="")  +
  scale_fill_brewer(palette="Blues")+
...
#Usage fraction by daily cooking times.
g111<- dplyr::select(ok_cooking_events_padded,stove_descriptions,day_month_year,HHID,duration_minutes,group) %>%
  ...
  ggtitle("Overall usage fraction by daily cooking time") + 
  facet_grid(~group,scales = "free", space = "free") + 
  labs(y="Usage fraction",x="")  +
  scale_fill_brewer(palette="Blues")+
  theme(legend.title = element_blank())
...
#Overall usage fractions by daily cooking times. 
g111_overall<- dplyr::select(ok_cooking_events_padded,stove_descriptions,day_month_year,HHID,duration_minutes,group) %>%
  dplyr::group_by(stove_descriptions,HHID) %>% #Group, then can do calculations on these groups.
  dplyr::summarise(avg_minutes_per_day_per_hhid=sum(duration_minutes,na.rm=TRUE)/length(unique(day_month_year))) %>% #Minutes per day per household with stove n.
  dplyr::group_by(stove_descriptions) %>% 
  dplyr::summarise(avg=mean(avg_minutes_per_day_per_hhid,na.rm=TRUE,keep_all=TRUE)) %>% #Average of those averages.
  dplyr::mutate(usefraction = avg/sum(avg)) %>% #Fraction of the average use of stove A over the total average time cooked at the household with all stoves A:N.  This means a single household with high usage of some stove will result in a high usage fraction overall.  
  dplyr::filter(usefraction>0.001) %>%
  dplyr::ungroup() %>%
  ggplot(aes(x=1, usefraction,label=sprintf("%0.2f", round(usefraction, digits = 2)))) +
  geom_col(aes(fill=stove_descriptions)) +
...
#Overall usage fraction by event durations.  
...
#Overall usage fraction by %stove days
g11b<- dplyr::select(ok_cooking_events_padded,stove_descriptions,day_month_year,use_flag) %>%
  ...
  geom_col(aes(fill=stove_descriptions))+
  geom_text(aes(group=stove_descriptions),position = position_stack(vjust = 0.5)) +
  ggtitle("Usage fraction by % stove days") + 
  labs(y="Usage fraction",x="")  +
  scale_fill_brewer(palette="Blues")+
  theme(legend.title = element_blank())
g11b
#ggsave(filename="../figures/usage_fraction_by_events.png", plot=g1ab,width = 6, height = 4)
...
gg_simul <- ggplot(simul_by_stove_combos %>% dplyr::filter(!is.na(group)),aes(x=stove_descriptions, y = percent_days_simul, color = stove_descriptions)) +
  stat_summary(fun.data = f, geom="boxplot") +
  geom_jitter(alpha = 0.25,position = position_dodge(width=0.75)) +
  labs(y="Percent of days with simultaneous use of stove",x="") + 
  facet_wrap(~group,scales = "free_x") + 
  stat_summary(fun.y=mean, colour="blue", geom="point", 
               shape=18, size=4,alpha = 0.5)+
  stat_summary(fun.data = give.mean5, geom = "text",colour="blue",alpha = 0.5,size = 3) + 
