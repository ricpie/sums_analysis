...
sumsarized_filtered <- filter_sumsarized(sumsarized,metadata,bad_files,HHID_remove,project_name) #HHID is grabbed from the filename.
```
   ...
   if (dim(temp)[1]>1 && any(temp$state==TRUE)) {
      ...
      #Organize/check/fix datetimes of sum placements and removals. If there is a start and end time available for the monitoring period, use it to keep the data from the file.  Disregard this if the datetime_removal is NA, since it means we don't have a fixed known end date.  In this case we assume the end of the file is the end of the monitoring preiod and keep all the data from the given file.  Provide the start and end date in the event-building loop below.
       if (max_time_file>end_date_range | min_time_file<start_date_range) {   #datetime_placed is changed to the local file value if it is below the start_date_range, and datetime_placed is too, according to the end_date_range
             ...
             qc_temp <- "out_of_placement_range"
...
stats_grouped_by_hhid_stove <- dplyr::group_by(ok_cooking_events_padded,stove_descriptions,HHID,group) %>%
    ...
    dplyr::summarise(events_per_day=sum(use_flag,na.rm=TRUE)/length(unique(day_month_year)),
                     ...
                     days_logged_per_group_by_stove=mean(logging_duration_days, na.rm = TRUE)) 
...
field_timeseries_plot(sumsarized_filtered, "stove_temp", "datetime", "HHID", "stove_description","qc") 

```

# Plot data time series, removing flagged files and households
* Plot usage rates (uses/day) by stove type, and region
```{r plot_nonflagged_timeseries, fig.width=10, fig.height=50}
#Plot time series for each household, colored by filename.  Filtered out bad qc data
field_timeseries_plot(dplyr::filter(sumsarized_filtered,grepl("ok",qc)),"stove_temp", "datetime", "HHID", "stove_description","qc") 

```
# Plot data time series, removing flagged files and households and houses without metadata.
* Plot usage rates (uses/day) by stove type, and region
```{r plot_ok_timeseries, fig.width=10, fig.height=50}
...
field_timeseries_plot(dplyr::filter(sumsarized_filtered,grepl("ok",qc)) %>% dplyr::filter(!is.na(stove_type)),
                      "stove_temp", "datetime", "HHID", "stove_description","qc") 
...
```{r plot_stove_usage3, fig.width=6, fig.height=4}
...
plotMonthlyFunc <- function(x,y, na.rm = TRUE, ...) {
    monthlyboxplots <- dplyr::filter(x,grepl(y,group,ignore.case=TRUE)) %>%
        dplyr::group_by(month_year,HHID,stove_descriptions) %>%
        dplyr::summarise(events_per_day_bymonth=sum(use_flag,na.rm=TRUE)/length(unique(day_month_year))) %>%
        ggplot(aes(x=month_year, y = events_per_day_bymonth,color = stove_descriptions)) +
...
for (i in 1:dim(stove_group_codes)[1]){
  plotMonthlyFunc(ok_cooking_events_padded,as.character(stove_group_codes$group[i]))
}
...
stovedays_test <- dplyr::group_by(ok_cooking_events,stove_descriptions,group,day_month_year) %>%
...
plotStoveDaysFunc <- function(x, na.rm = TRUE, ...) {
  ...
  g7 <-dplyr::group_by(ok_cooking_events_padded,day_month_year,stove_descriptions) %>%
