...
parameter_filename <- "parameter_file_NAMASolar.R"  #Format example is :"parameter_file_HCCP_Ethiopia.R"
...
if (( reimport_data == TRUE) |  (!file.exists("../r_files/sumsarized_filtered.RDS"))) {
  sumsarized_filtered <- ldply(unique(sumsarized$fullsumsarizer_filename),filter_sumsarized_ldply,sumsarized,metadata,bad_files,HHID_remove,
                               project_name,stove_codes,.parallel = TRUE)  %>% #HHID is grabbed from the filename.
    dplyr::mutate(sumsarized_filtered,file_indices = match(fullsumsarizer_filename, unique(fullsumsarizer_filename))) %>%
...
sumsarized_filtered <-dplyr::left_join(sumsarized_filtered,stove_group_codes,by = "group") %>%
  ...
  dplyr::mutate(region = gsub('.csv','',region)) 

#If there is no region variable, set it to 'group' or something
