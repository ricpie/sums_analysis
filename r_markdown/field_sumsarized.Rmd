...
gg <- field_timeseries_plot(sumsarized_filtered, "stove_temp", "datetime", "HHID", "stove_descriptions","qc") 
gg
```
...
g111<- dplyr::group_by(ok_cooking_events_padded, stove_descriptions, day_month_year, HHID, group) %>%
  ...
  #facet_grid(stove_use_category~group,scales = "free", space = "free") + 
  #stat_summary(fun.data = give.n, geom = "text") + 
  facet_grid( group ~ stove_descriptions,scales = "free", space = "free", labeller = label_wrap_gen(width = 14, multi_line = TRUE)) +
...
g3<-  dplyr::group_by(ok_cooking_events_padded,stove_descriptions,HHID,group) %>%
  ...
  facet_grid( ~group,scales = "free", space = "free", labeller = label_wrap_gen(width = 14, multi_line = TRUE)) +
...
g5 <- ggplot(ok_cooking_events, aes(x=day_of_week, y = duration_minutes)) + 
    ...
    facet_wrap( group ~ stove_descriptions,scales = "free_x", labeller = label_wrap_gen(width = 20, multi_line = TRUE)) +
...
#Overall usage fraction by daily cooking times and by group.
...
gg_stack <- ggplot(stacking_stovedays%>%dplyr::filter(!is.na(group)),aes(x=stove_descriptions, y = percent_stovedays,color = str_wrap(stove_descriptions,25))) +
    stat_summary(fun.data = f, geom="boxplot") +
    geom_jitter(alpha = 0.25,position = position_dodge(width=0.75)) +
    ...
    scale_x_discrete(labels = function(x) str_wrap(x, width = 20))+
    theme(axis.text.x = element_text(angle = 60, hjust = 1),legend.title = element_blank(),legend.key.height=unit(2, "cm"))
gg_stack
...
gg_simul <- ggplot(simul_by_stove_combos %>% dplyr::filter(!is.na(group)),aes(x=stove_descriptions, y = percent_days_simul, color =  str_wrap(stove_descriptions,25))) +
    stat_summary(fun.data = f, geom="boxplot") +
    geom_jitter(alpha = 0.25,position = position_dodge(width=0.75)) +
