...
```{r kpt_sums_comparison,echo=TRUE,fig.width=7, fig.height=10}
...
kpt_start <- dplyr::mutate(kpt_data_all,startdatetime=as.POSIXct(paste(Startdate, substr(Starttime,1,8)), format="%Y-%m-%d %H:%M:%S")) %>%
  dplyr::select(HHID,startdatetime,`AverageBiomassKG/day/SA`,RegionName) %>%
...
cooking_kpt <- dplyr::left_join(dplyr::mutate(ok_cooking_events_padded,HHID = as.numeric(HHID)),kpt_start,by='HHID')  %>%
  ...
  dplyr::mutate(during_kpt_flag = if_else(start_time>startdatetime & start_time<enddatetime,TRUE,FALSE)) #Keep events within start and end time of KPTs

# Compare the behaviors with the average use from the rest of the time.
