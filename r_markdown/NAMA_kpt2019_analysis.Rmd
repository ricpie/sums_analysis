...
all_plots <- function(kpt_data_all){
  ...
  region_LPG_SAplot <- kpt_plot_onevar(kpt_data_all, x_var="group", y_var = "`AverageLPGMJ/day/SA`", facet_var1 = "RegionName", plot_title="", xlabel="",ylabel = "Energia de GLP por día por adulto estándar [MJ/día/SA]" ,give.mean_minutes_LPG); print(region_LPG_SAplot)
  ...
  national_kg_SAplot <- kpt_plot_group(kpt_data_all, x_var="`group`", y_var = "`AverageBiomassKG/day/SA`",  plot_title="", xlabel="",ylabel = "Consumo de biomasa [kg/día/SA]", giver=give.mean_minutes_kg ); print(national_kg_SAplot)
...
```{r kpt_sums_comparison,echo=TRUE,fig.width=7, fig.height=5}
...
kpt_start <- dplyr::mutate(kpt_data_all,kptstartdatetime=as.POSIXct(paste(Startdate, substr(Starttime,1,8)), format="%Y-%m-%d %H:%M:%S")) %>%
  dplyr::select(HHID,kptstartdatetime,`AverageBiomassKG/day/SA`,RegionName,group) %>%
  dplyr::mutate(kptenddatetime = kptstartdatetime + 3*86400)
...
cooking_kpt <- dplyr::left_join(dplyr::mutate(ok_cooking_events_padded,HHID = as.numeric(HHID)),kpt_start,by='HHID')  %>%
  dplyr::filter(!is.na(kptstartdatetime)) %>% # Remove rows that don't have start time (ie. no HHID match was found)
  dplyr::mutate(during_kpt_flag = if_else(start_time<kptstartdatetime ,'Antes_de_KPT',
                                          if_else(start_time>kptenddatetime,'Despues_de_KPT','Durante_KPT'))) #Keep events within start and end time of KPTs
...
representativeness <-   dplyr::group_by(cooking_kpt,stove_descriptions,HHID,group.x,during_kpt_flag) %>%
  ...
  facet_grid(group.x~stove_descriptions,labeller = labeller(facet_var = label_wrap_gen(width = 15))) +
  stat_compare_means(method = "t.test") +
