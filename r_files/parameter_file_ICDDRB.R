...
path_tracking_sheet <- "SUMs_tracking form icddrb.xlsx"

project_name <- "ICDDRB"
#Text associating a given stove code with the full name to use in figures.
stove_codes <- data.frame(stove = as.factor(c("TMS","TMT","TMS2","TMT2","LPG","LPG2","FMS","ICS", "AMB")),
                          stove_descriptions = as.factor(c("1st Traditional Mud Stove, single burger",
                                                           "1st traditional mud stove, two burner",
                                                           "Secondary single burner traditional mud stove",
                                                           "Secondary two burner traditional mud stove",
                                                           "Liquified petroleum gas",
                                                           "Secondary Liquified petroleum gas",
                                                           "Floating mud stove","Local improved wood stove",
                                                           "Ambient")))
...
end_date_range <- "2019-12-1" #Do not include data after this in the analysis
...
bad_files <- paste(c("Pre_Pilot-04_TMS_DL-01","|Pre_Pilot-04_TMS_DL-01","|Pre_Pilot-04_TMS_DL-01"
                     ,"|Pre_Pilot-04_TMS_DL-01"),collapse="")

# Exclude data from the following households from the entire analysis. e.g.SHO_03 is removed
HHID_remove <- paste(c("^SHO_03$","|^SHO_04$","|^MUS_01$"),collapse="")
